...
// Cada fila crea un usuario con rol EMPLEADO, sin departamento (se asigna
// después manualmente). Si el username ya existe, esa fila se omite y se
// reporta al final en vez de detener toda la carga.
...
export const importUsers = async (
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const exists = await prismaClient.user.findUnique({ where: { username } });
    if (exists) {
